...
def eval_fn(valid_dataloader, model, device):
    ...
    save_roc_curve(fin_labels, [prob[0] for prob in fin_outputs], 'mh_roc_curve_electra')
...
model = ELECTRAModel()
model = nn.DataParallel(model) ##Speeds up training uses all 7GPUs
...
plt.savefig('mh_electra_confusion_matrix.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_electra_accuracy_history.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_electra_loss_history.png')
plt.close()
