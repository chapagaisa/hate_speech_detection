...
class MetaHateDatSet(Dataset):
  ...
  def __getitem__(self, item):
    ...
    assert len(input_ids) == self.max_len, f"Length of input_ids ({len(input_ids)}) does not match max_len ({self.max_len})"
    assert len(attention_mask) == self.max_len, f"Length of attention_mask ({len(attention_mask)}) does not match max_len ({self.max_len})"
...
def eval_fn(valid_dataloader, model, device):
    ...
    save_roc_curve(fin_labels, [prob[0] for prob in fin_outputs], 'mh_roc_curve_gpt2')
...
train_dataloader = DataLoader(train_data, batch_size=Config.TRAIN_BATCH_SIZE, num_workers=0)
valid_dataloader = DataLoader(valid_data, batch_size=Config.VALID_BATCH_SIZE, num_workers=0)
test_dataloader = DataLoader(test_data, batch_size=Config.VALID_BATCH_SIZE, num_workers=0)

model = GPT2Model()
model = nn.DataParallel(model) ##Speeds up training uses all 7GPU
...
plt.savefig('mh_gpt2_confusion_matrix.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_gpt2_accuracy_history.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_gpt2_loss_history.png')
plt.close()
