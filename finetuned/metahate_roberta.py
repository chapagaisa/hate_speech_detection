...
class Config:
    ...
    TOKENIZER = transformers.RobertaTokenizer.from_pretrained(ROBERTA_PATH, do_lower_case=True)
    MODEL = transformers.RobertaModel.from_pretrained(ROBERTA_PATH, return_dict=False)
...
class RoBERTaModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        hidden_states = roberta_outputs[0]  # Last hidden state (bs, seq_len, dim)
        bo = self.roberta_drop(hidden_states[:, 0])  # pooled output (bs, dim)
        output = self.out(bo)
        return output
...
def eval_fn(valid_dataloader, model, device):
    ...
    save_roc_curve(fin_labels, [prob[0] for prob in fin_outputs], 'mh_roc_curve_roberta')
...
model = RoBERTaModel()
model = nn.DataParallel(model)  # Speeds up training uses all GPUs
...
plt.savefig('mh_roberta_confusion_matrix.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_roberta_accuracy_history.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_roberta_loss_history.png')
plt.close()
