...
class Config:
    ...
    TOKENIZER = transformers.AutoTokenizer.from_pretrained(DISTILBERT_PATH, do_lower_case=True)
    MODEL = transformers.AutoModel.from_pretrained(DISTILBERT_PATH, return_dict=False)


class MetaHateDatSet(Dataset):
    def __init__(self, data_path):
        self.data = pd.read_csv(data_path).fillna('none')
        #self.data.label = self.data.label.apply(lambda x: 1 if x == "1" else 0)
...
class DistilBERTModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        dbo = self.distilbert_drop(mean_last_hidden_state)
        output = self.out(dbo)
        return output
...
def eval_fn(valid_dataloader, model, device):
    ...
    save_roc_curve(fin_labels, [prob[0] for prob in fin_outputs], 'mh_roc_curve_distilbert')    
...
model = nn.DataParallel(model)  # Use this if you are utilizing multiple GPUs
...
plt.savefig('mh_distilbert_confusion_matrix.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_distilbert_accuracy_history.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_distilbert_loss_history.png')
plt.close()
