...
def eval_fn(valid_dataloader, model, device):
  ...
  save_roc_curve(fin_labels, [prob[0] for prob in fin_outputs], 'mh_roc_curve_t5')	
...
plt.savefig('mh_t5_confusion_matrix.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_t5_accuracy_history.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_t5_loss_history.png')
plt.close()
