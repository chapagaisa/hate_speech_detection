...
class XLNetClassificationModel(nn.Module):
    def __init__(self):
        ...
        self.xlnet = Config.MODEL
        self.xlnet_drop = nn.Dropout(0.3)
        self.out = nn.Linear(self.xlnet.config.hidden_size, 2)

    def forward(self, input_ids, attention_mask):
        xlnet_outputs = self.xlnet(input_ids=input_ids, attention_mask=attention_mask)
        hidden_states = xlnet_outputs[0]  # Last hidden state (bs, seq_len, dim)
        bo = self.xlnet_drop(hidden_states[:, 0])  # pooled output (bs, dim)
        output = self.out(bo)
        return output
...
def eval_fn(valid_dataloader, model, device):
    ...
    save_roc_curve(fin_labels, [prob[0] for prob in fin_outputs], 'mh_roc_curve_xlnet')
...
model = XLNetClassificationModel()
...
plt.savefig('mh_xlnet_confusion_matrix.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_xlnet_accuracy_history.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('mh_xlnet_loss_history.png')
plt.close()
