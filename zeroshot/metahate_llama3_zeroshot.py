...
class Config:
    ...
    pipeline = transformers.pipeline("zero-shot-classification", model=model_id, model_kwargs={"torch_dtype": torch.float16}, device=7)
...
def classify_text(text):
    response = Config.pipeline(text, candidate_labels = ["hate", "no hate"])
    #print(response)
...
def eval_fn(dataloader):
    ...
    save_roc_curve(fin_labels, [prob for prob in fin_outputs], 'mh_roc_curve_llama3_zeroshot')
...
plt.savefig('mh_llama3_zeroshot_confusion_matrix.png')
plt.close()
